      ...
      >
        ...
        <div>
          <fieldset :class="`${prefixCls}-item-fieldset`">
            <legend :class="`${prefixCls}-item-fieldset-legend`">key</legend>

            <a-input placeholder="key" v-model:value="item.key"> </a-input>
          </fieldset>
        </div>
        <div>
          <fieldset :class="`${prefixCls}-item-fieldset`">
            <legend :class="`${prefixCls}-item-fieldset-legend`">value</legend>
...
(async function init() {
  ...
  Object.keys(modelValue.value).forEach((v) => {
    ...
    if (
      Object.prototype.toString.call(modelValue.value[v]) === "[object String]"
    ) {
      valueType = "string";
    }
    if (
      Object.prototype.toString.call(modelValue.value[v]) === "[object Number]"
    ) {
      valueType = "number";
    }

    if (
      Object.prototype.toString.call(modelValue.value[v]) === "[object Boolean]"
    ) {
      valueType = "boolean";
    }

    if (
      Object.prototype.toString.call(modelValue.value[v]) === "[object Object]"
    ) {
      valueType = "object";
    }
