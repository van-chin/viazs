<template>
	...
	<div :class="`${prefixCls}-wrapper`">
			...
			<a-table :class="prefixCls" :id="tableId" v-bind="$attrs" ref="tableRef" :data-source="modelLists || []" bordered
				...
				<template v-for="(_, name) in $slots" v-slot:[name]="slotProps">
                  ...
                  /> -->
									...
									<icon v-if="schemes?.length >= 1" @click="openColumnsSet"
										icon="ant-design:setting-outlined" :inline="true" class="cursor-pointer">
									</icon>
								</a-space>
							</div>
						</template>
		...
		<div :class="`${prefixCls}-footer-bar flex justify-between items-center bg-white px-2 h-[50px] `"
			v-if="footerBar">
			<div>
				<slot name="footer-action"></slot>
			</div>

			<a-pagination v-bind="paginations" @change="onPaginationChange"
				:show-total="(total: number) => `总共 ${total} 条`" show-size-changer :responsive="true"
...
import { VzTableProps, DragEndParams, VzTableColumn,SortableEndParams } from "@viaz/types";
...
const { width: tableWrapperWidth, height: tableWrapperHeight } = useElementSize(contentElRef);
