<template>
		...
		<a-space :size="0">
			...
			<template v-for="(action, index) in actions">
				<template
					v-if="
						action.position ===
						PageCurdActionPositionEnum.TABLE_ROW_OPERATIONS
					"
				>
	...
	>
		<div :class="`${prefixCls}-filter`" v-if="!initialing" ref="filterEl">
			<vz-form-renderer
				:data="searcher"
				:ref="toRef('searcherFromRef')"
				@keywords-pressEnter="onKeywordsPressEnter"
			>
			</vz-form-renderer>
			<div
				:class="`${prefixCls}-filter-condition-bar`"
				v-if="conditionBar"
			>
				<fieldset>
					<legend>筛选条件</legend>
						...
						>
							...
							<template #overlay>
								<a-menu>
									...
									<a-menu-item>
										<a-checkbox
											v-model:checked="immediateSearch"
											>立即搜索</a-checkbox
										>
									</a-menu-item>
									<a-menu-item>
...
<script lang="ts" setup>
	...
	const { refs, toRef } = useRefs<{
		searcherFromRef: InstanceType<typeof HTMLElement>;
		filterFromRef: InstanceType<typeof HTMLElement>;
	}>();
	...
	defineExpose({
		refs: refs,
		filterFromRef: refs.filterFromRef,
		searcherFromRef: refs.searcherFromRef,
		filterForm,
	});
</script>
