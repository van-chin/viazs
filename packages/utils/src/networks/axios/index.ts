...
import { useGlobalSetting, useMessage } from "@viaz/hooks";
import { RequestEnum, ResultEnum, ContentTypeEnum } from "@viaz/enums";
import { REFRESH_TOKEN_KEY } from "@viaz/constants";
import { isString, isUnDef, isNull, isEmpty } from "../../is";

import { setObjToUrlParams, deepMerge } from "../../oths";
...
const transform: AxiosTransform = {
	...
	requestInterceptors: (config, options) => {
		// 请求之前处理config
		// const token = getToken("ACCESS_TOKEN__");

		const token = localStorage.getItem("ACCESS_TOKEN_HIS");
		const refreshToken = localStorage.getItem("REFRESH_TOKEN_HIS");

		// console.info("requestInterceptors.config =>", config);

		// console.info("requestInterceptors.token =>", token);
		// const refreshToken = getToken("REFRESH_TOKEN__");
		...
		if (
			refreshToken &&
			(config as Recordable<any>)?.requestOptions
				?.withAuthorizationRefreshToken === true
		) {
		...
		if (
			token &&
			(config as Recordable<any>)?.requestOptions
				?.withAuthorizationAccessToken === true
		) {
